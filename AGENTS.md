...
- **Version must be kept in sync** between `package.json` and `public/manifest.json` (currently 0.6.4). Bump both. CI (`.github/workflows/release.yml`) fails if they differ, and requires release tags to be exactly `v<version>`.
